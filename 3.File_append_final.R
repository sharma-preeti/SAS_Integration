...
op_dir <- "E:\\Tiger\\preeti\\SAS Integration\\3. Tool_Input/Tool_Input_v3"
...
data_write <- function(file,consolidated_Df){
  ...
  consolidated_Df$Retailer <- ifelse(consolidated_Df$Retailer=="Shoprite","ShopRite",consolidated_Df$Retailer)
...
for (Retailer in Retailers_1){
  ...
  tryCatch({
    ...
    Model_Coefficients_EDLP_TPR_Tranformed_fnl = merge(Model_Coefficients_EDLP_TPR_Tranformed_fnl,unique(New_Model_Coeff_fnl[,c("PPG_Cat",
                                                                    "Market","Model_flag","Modelled_PPG_Description")]),by.x=c("Market","PPG_Cat","PPG_Item"),
                                                       by.y=c("Market","PPG_Cat","Modelled_PPG_Description"),all.x=T)
    
    Model_Coefficients_EDLP_TPR_Tranformed_Optimizer_fnl = merge(Model_Coefficients_EDLP_TPR_Tranformed_Optimizer_fnl,unique(New_Model_Coeff_fnl[,c("PPG_Cat",
                                                                                                                                "Market","Model_flag","Modelled_PPG_Description")]),
                                                                 by.x=c("Market","PPG_Cat","PPG_Description"),
                                                       by.y=c("Market","PPG_Cat","Modelled_PPG_Description"),all.x=T)
